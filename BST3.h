...
// Function that return a iterator with a pointer to the next node that has a greater 
// key or the node with the given min key.
template <typename TKey>
typename bst<TKey>::iterator bst<TKey>::lower_bound(const TKey &key)
{
  bst<TKey>::iterator it = begin();

  while ((it != NULL) && (*it < key)) { ++it; }

  return it;
}

// Function that return a iterator with a pointer to the next node that has a greater 
// key or NULL if the node dont exist.
template <typename TKey>
...
// Function that erase the nodes inside the range, bounded by the two given iterators
template <class TKey>
void bst<TKey>::erase(iterator i0, iterator i1)
{
  vector<TKey> v;
  ...
  if (i0 != NULL)
  {
    ...
    typename std::vector<TKey>::iterator it = v.begin();

    while (it != v.end())
    {
      Troot = erase(Troot, *it);
      ++it;
    }    
  }
}
